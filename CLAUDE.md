...
- [`sandbox-enablement.md`](docs/learnings/sandbox-enablement.md) — enabling Cowork's bwrap jail and Chromium's renderer sandbox under Ubuntu 24.04's `apparmor_restrict_unprivileged_userns` restriction: the `.deb` postinst auto-config, the Wayland `--no-sandbox`/Xorg requirement, fresh VM + host runbooks, KVM-via-nested-virt, and the `--doctor` + `pgrep [e]lectron` verification recipes
